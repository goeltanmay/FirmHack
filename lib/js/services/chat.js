function Chat (sb) {
	...
	this.init_with_chat_channel = function (chat_chan) {
		chat_channel = chat_chan;
	}

	this.get_chat_channel = function () {
		// body...
		return chat_channel;
	}
	...
	sb.GroupChannel.createChannelWithUserIds([user.user_id], true, user.nickname, "", null, function(channel, error) {
		    if (error) {
		        console.error(error);
		        return;
		    }
